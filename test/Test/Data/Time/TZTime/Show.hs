...
import Data.Time (LocalTime(..), TimeOfDay(..), ZonedTime(..), minutesToTimeZone)
...
unit_TZOffsetError :: Assertion
unit_TZOffsetError = do
  let tz = TZI.fromLabel TZI.America__Winnipeg

  first displayException
    (TZT.fromZonedTimeStrict tz (ZonedTime (LocalTime (YearMonthDay 2022 3 13) (TimeOfDay 2 15 0)) cst)) @?= Left
    "The local time 2022-03-13 02:15:00 is invalid in the time zone \"America/Winnipeg\"."

  first displayException
    (TZT.fromZonedTimeStrict tz (ZonedTime (LocalTime (YearMonthDay 2022 11 6) (TimeOfDay 1 15 0)) (minutesToTimeZone 60))) @?= Left
    "The local time 2022-11-06 01:15:00 does not occur at the offset +01:00 in the time zone \"America/Winnipeg\". Valid offsets: -05:00, -06:00."
